...
class User(db.Model):
    ...
    height = db.Column(db.Float, nullable=True, default=None)  # in cm
    weight = db.Column(db.Float, nullable=True, default=None)  # in kg
...
class Exercise(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(120), nullable=False)
    muscle_group = db.Column(db.String(50), nullable=False)  # chest, back, shoulders, etc.
    specific_muscle = db.Column(db.String(50), nullable=False)  # upper_chest, front_delt, etc.
    is_default = db.Column(db.Boolean, default=True)  # True for pre-loaded exercises
    created_by = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)  # NULL for default exercises
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    creator = db.relationship('User', backref='custom_exercises')
